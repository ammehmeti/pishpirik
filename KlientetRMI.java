...
public class KlientetRMI extends JFrame implements ActionListener, Runnable {
						...
						if (lojtaret.size() >= 2) {
							...
							if (!aperfundoi)
								if (kontainer.size() >= 1) {
									...
									ImageIcon imgThisImg1 = new ImageIcon("res/none.jpg");
									shfaqkontainer.setIcon(imgThisImg1);
								}
	...
	private void setServer(String ipAddress) {
		try {
			Registry regjistri = LocateRegistry.getRegistry(ipAddress, 10005);
			c = (PishpirikChat) regjistri.lookup("lidhja");
		}
		/*
